...
if [ "$CSP" = "aws" ]; then
  if [[ ! -f "aws_disk.vmdk" ]]; then
    ...
    curl -O https://f004.backblazeb2.com/file/cvm-base-images/06082025/aws_disk.vmdk
  fi
elif [ "$CSP" = "azure" ]; then
  if [[ ! -f "azure_disk.vhd" ]]; then
    echo "⌛ Downloading azure_disk.vhd..."
    curl -O https://f004.backblazeb2.com/file/cvm-base-images/06082025/azure_disk.vhd
  fi
elif [ "$CSP" = "gcp" ]; then
  if [[ ! -f "gcp_disk.tar.gz" ]]; then
    echo "⌛ Downloading gcp_disk.tar.gz..."
    curl -O https://f004.backblazeb2.com/file/cvm-base-images/06082025/gcp_disk.tar.gz
  fi
else
