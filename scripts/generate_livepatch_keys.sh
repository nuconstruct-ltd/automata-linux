...
if [ -f "$LIVEPATCH_KEY_NAME.key" ] && [ -f "$LIVEPATCH_KEY_NAME.crt" ]; then
  ...
  # If aws-uefi-blob.bin exists, remove it, it needs to be regenerated later on
  if [ -f "secure_boot/aws-uefi-blob.bin" ]; then
    echo "Removing existing aws-uefi-blob.bin as it is outdated."
    rm "secure_boot/aws-uefi-blob.bin"
  fi
fi